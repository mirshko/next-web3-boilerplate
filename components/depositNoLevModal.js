...
const DepositWithdrawalModal = ({
  ...
  useEffect(() => {
    const process = async () => {
      if (!token0Contract || !token1Contract || !account || !destContract)
        return;
      try {
        if (runningTx == 0) {
          let result = await token0Contract.allowance(
            account,
            destContract.address
          );
          setLpAllowance0(result);
          console.log("allow0", result.toString());
          result = await token1Contract.allowance(
            account,
            destContract.address
          );
          ...
          if (lpAllowance0.lt(token0Amount)) {
            const { hash } = await token0Contract.approve(
              destContract.address,
              ethers.constants.MaxUint256
            );

            showSuccessNotification(
              "Asset allowance approved",
              "Asset allowance approved successful",
              hash
            );
          ...
          if (lpAllowance1.lt(token1Amount)) {
            const { hash } = await token1Contract.approve(
              destContract.address,
              ethers.constants.MaxUint256
            );
          ...
          if (isLpV2) {
            ...
            let result = await zapboxContract.zapIn(
              lendingPool.poolId,
              token0.address,
              token0Amount,
              token1.address,
            ...
            const { hash } = await zapboxTRContract.zapIn(
              lendingPool.poolId,
              rangeAddress,
              token0Amount,
              token1Amount
  ...
  return (
    <>
      <Button
        ...
        onClick={openModal}
        size={"default"}
        disabled={token0Amount == 0 && token1Amount == 0}
      >
        Farm
      </Button>
      <Modal
