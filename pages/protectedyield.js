...
function ProtectedYield() {
  ...
  var insuranceAsset = useAssetData(
    selectedAsset.insuranceAsset,
    lendingPool.address
  );
  ...
  const [isVisibleCollateral, setVisibilityCollateral] = useState(
  ...
  const farm = async () => {
    ...
    if (amount0 > token0.tvl || amount1 > token1.tvl) {
      console.log("Not enough assets to borrow");
      return;
    }
    let amounts = [
      ethers.utils.parseUnits(
        amount0.toFixed(token0.decimals),
        token0.decimals
      ),
      ethers.utils.parseUnits(
        amount1.toFixed(token1.decimals),
        token1.decimals
      ),
    ];
    ...
    try {
      ...
      console.log(
        "flashhh",
        ADDRESSES["rangerPositionManager"],
  ...
  const buyInsurance = async () => {
    ...
    console.log(
      ...
      ((coverage / 100) * selectedAsset.deposited * selectedAsset.oraclePrice) /
        insuranceAsset.oraclePrice,
      parseFloat(
        ((coverage / 100) *
          selectedAsset.deposited *
          selectedAsset.oraclePrice) /
          insuranceAsset.oraclePrice
      ).toFixed(18),
      18
    );
    ...
    let params = abi.encode(
      ["uint", "uint8", "address", "address[]"],
      [0, lendingPool.poolId, account, [ethers.constants.AddressZero]]
    );
    try {
    ...
    <div>
      <Typography.Title>
        ...
        <VaultsDropdown
          ...
          currentVault={vaults[currentVault]}
          size="30px"
        />
      </Typography.Title>
      ...
      <Row gutter={16} style={{ marginTop: 16, marginLeft: 24 }}>
        ...
        <Col md={8}>
          <Card>
            ...
            The AMM yield farmer collects good yield but is exposed to the asset
            price going down.
            <br />
            <br />
          ...
          <Modal
            title="Protected Yield Farming"
            open={isHelpVisible}
            onChange={() => {
              toggleHelpVisible(!isHelpVisible);
            }}
            onOk={() => {
              toggleHelpVisible(!isHelpVisible);
            }}
          ...
          >
            ...
            <img
              ...
              style={{ float: "left", marginTop: 12, marginRight: 12 }}
            />
            <br />
            <br />
            Pick some ROE perpetual options to partially hedge your position in
            case of market dump.
            <br />
            <br />
          </Modal>
      ...
      <Row gutter={16} style={{ marginTop: 24 }}>
        <Col xl={8} type="flex">
          ...
          <Card
            title="No Leverage"
            bordered={false}
            style={{ marginLeft: 32, width: 330, height: 300 }}
            ...
            <div style={{}}>
              <span>
                ...
                <span style={{ float: "right", fontSize: "smaller" }}>
                  Wallet: {parseFloat(token0.wallet).toFixed(3)}
                </span>
              </span>
              ...
              <span style={{ width: "100%" }}>
                {token1.name}
                <span style={{ float: "right", fontSize: "smaller" }}>
                  Wallet: {parseFloat(token1.wallet).toFixed(3)}
                </span>
              </span>
              <br />
              <InputNumber
              ...
              <Checkbox
                ...
                onChange={(e) => {
                  setUseNativeToken(e.target.checked);
                }}
                style={{ marginBottom: 12 }}
                disabled
              >
                Add <strong>WETH</strong> / ETH
              </Checkbox>
              <br />
        ...
        <Col xl={8} type="flex">
          <Typography.Title level={2}>
            3. Leverage? &nbsp;
            <Tooltip title="Careful with that slider!">
              <ExclamationCircleOutlined style={{ fontSize: "smaller" }} />
            </Tooltip>
          </Typography.Title>
          ...
          <Card
            title={
              ...
              (availableCollateral * percentDebt) / 100
            }
            bordered={false}
            style={{ marginLeft: 32, width: 330, height: 300 }}
            bodyStyle={{ height: "100%" }}
          >
            <h5 style={{ marginTop: 0, marginBottom: 0, color: "grey" }}>
              Leverage
            </h5>
            <div
              style={{
        ...
        <Col xl={8} type="flex">
          <Typography.Title level={2}>
            ...
            {insuranceAsset.name ? (
              <span style={{ float: "right", marginRight: 12 }}>
                {insuranceAsset.name.split("-")[1]}-Puts
              </span>
            ) : null}
          </Typography.Title>
          ...
          <Card
            title={
              "Farming Position: $" +
              selectedAsset.deposited * selectedAsset.oraclePrice
            }
            bordered={false}
            style={{ marginLeft: 32, width: 330, height: 300 }}
            bodyStyle={{ height: "100%" }}
          >
            ...
            >
              ...
              <span style={{ color: "lightgreen", fontWeight: "bold" }}>
                {(
                  (coverage / 100) *
                  parseFloat(insuranceAsset.debtApr)
                ).toFixed(2)}
                %
              </span>
            </div>
